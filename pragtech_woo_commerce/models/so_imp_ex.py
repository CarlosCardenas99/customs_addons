                                    ...
                                    if order_line_list:
                                        ...
                                        for sl in ele.get('shipping_lines'):
                                            ...
                                            if shipping and shipping.product_id:
                                                ...
                                                # if shipping_so_line.qty_to_invoice > 0:
            ...
            if res_partner:
                ...
                if ele.get('meta_data'):
                    for record in ele.get('meta_data'):
                        if record.get('key') == '_wc_shipment_tracking_items':
                            tracking_info = self.env['tracking.info']
                            if record.get('value'):
                                if isinstance(record.get('value'), list):
                                    timestamp = record.get('value')[0].get('date_shipped')
                                    dt_object = datetime.fromtimestamp(int(timestamp))
                                    tracking_dict = {}
                                    tracking_dict.update({
                                        'tracking_number': record.get('value')[0].get(
                                            'tracking_number'),
                                        'tracking_provider': record.get('value')[0].get(
                                            'tracking_provider'),
                                        'date_shipped': dt_object.date(),
                                        'order_id': so_obj.id
                                    })
                                    tracking = tracking_info.sudo().create(tracking_dict)
                                    tracking_list.append(tracking.id)

                                else:
                                    if isinstance(record.get('value'), dict):
                                        for tracking in record.get('value').values():
                                            timestamp = tracking.get('date_shipped')
                                            dt_object = datetime.fromtimestamp(int(timestamp)).date()
                                            tracking_dict = {}
                                            tracking_dict.update({
                                                'tracking_number': tracking.get(
                                                    'tracking_number'),
                                                'tracking_provider': tracking.get(
                                                    'tracking_provider'),
                                                'date_shipped': dt_object,
                                                'order_id': so_obj.id
                                            })
                                            tracking = tracking_info.sudo().create(tracking_dict)
                                            tracking_list.append(tracking.id)
                ...
                for i in ele.get('line_items'):
                    ...
                    if res_product:
                        ...
                        order_line_list.append(create_p)
                if tracking_list:
                    so_obj.tracking_info_ids = [(6, 0, tracking_list)]
                if order_line_list:
                    for sl in ele.get('shipping_lines'):
                            ...
                            if isinstance(record.get('value'), list):
                                ...
                                if isinstance(record.get('value'), dict):
                                    for tracking in record.get('value').values():
                                        ...
                                        tracking_list.append(tracking.id)
            if tracking_list:
                sale_order.tracking_info_ids = [(6, 0, tracking_list)]
